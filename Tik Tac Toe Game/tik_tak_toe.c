...
void win_check(int M, int N, int G[][5], int K, int *win, int *winner)
{
    int sum1, sum2;
    ...
    for(int i = 0; i < M; i++) {
        ...
        if(sum1 == K) { // Player 1 win condition
            *win = 1;
            *winner = 1;
            return;
        }
        else if(sum2 == K) { // Player 2 win condition
            *win = 1;
            *winner = 2;
            return;
        }
    ...
    for (int i = 0; i <= M - K; i++) {
        for (int j = 0; j <= N - K; j++) {
            if (G[i][j] == 1 && G[i + 1][j + 1] == 1 && G[i + 2][j + 2] == 1) { //player 1 win condition
                *win = 1;
                *winner = 1;
                return;
            } else if (G[i][j] == 2 && G[i + 1][j + 1] == 2 && G[i + 2][j + 2] == 2) {//player 2 win condition
                *win = 1;
                *winner = 2;
                return;
            }
    ...
    for (int i = 0; i <= M - K; i++) {
        for (int j = K - 1; j < N; j++) {
            if (G[i][j] == 1 && G[i + 1][j - 1] == 1 && G[i + 2][j - 2] == 1) {
                ...
                *winner = 1;
                return;
            } else if (G[i][j] == 2 && G[i + 1][j - 1] == 2 && G[i + 2][j - 2] == 2) {
                *win = 1;
                *winner = 2;
                return;
            }
...
int main(void)
{
    ...
    const int K=3; //number of squares to win
    int G[M][N]={0};
    ...
    while (win==0){
        ...
        separator();
        field(M,N,G); 
        if(mv>=K){ //Check for possible win after 4 moves
            win_check(M, N, G, K, &win, &winner);
            if(win){
                break;
            }
        }
